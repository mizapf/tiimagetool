...
/*************************** Low-level routines *****************************/
	...
	private static void writeTrackInArray(byte[] image, int density, int tracksperside, int sectpertrack, int nTrack, int nSide, boolean sdf) {
		...
		if (sdf) {
			nTrackLength = sectpertrack * 256;
			track = new byte[nTrackLength];

			// Calculate track offset
			if (nSide == 0) {
				nOffset = sectpertrack * 256 * nTrack;
			}
			else {
				nOffset = (2*tracksperside - 1 - nTrack)*sectpertrack*256;
			}

			for (int i=0; i < track.length; i++) track[i] = (byte)0;
		}
		else {
			if (density==0) nTrackLength = 16 + sectpertrack*334 + 231;
			else nTrackLength = 40 + sectpertrack*340 + 712;
			track = new byte[nTrackLength];
			for (int i=0; i < track.length; i++) track[i] = (byte)0;

			if (density==0) {
				for (int i=nTrackLength-231; i < nTrackLength; i++)	 track[i] = (byte)0xff;
			}
			else {
				for (int i=0; i < 40; i++) track[i] = (byte)0x4e;
				for (int i=nTrackLength-712; i < nTrackLength; i++)	 track[i] = (byte)0x4e;
			}
			
			nOffset = (nSide * tracksperside + nTrack)*nTrackLength; 
		}
		
		// Write all sectors
		byte[] sectordata = new byte[256];
		for (int i=0; i < sectordata.length; i++) sectordata[i] = (byte)0xe5;
		
		for (int nSector = 0; nSector < sectpertrack; nSector++) {
			writeSectorInArray(track, sectordata, sectpertrack, nSide, nTrack, nSector, sdf);
		}
		
		System.arraycopy(track, 0, image, nOffset, nTrackLength);
	}
	
	private static void writeSectorInArray(byte[] image, byte[] sector, int sectpertrack, int nSide, int nTrack, int nSector, boolean sdf) {
		int nOffset = 0;
		
		if (sdf) {
			nOffset = nSector*256;			
			int i=nOffset;							
			for (int j=0; j < 256; j++) image[i++] = sector[j];
		}
		else {
			int index = 0;		
			if (sectpertrack < 10) {
				index = (nSector*4 + nTrack*3) % sectpertrack;
				nOffset = 16 + index*334;
				int i=nOffset;				
				for (int j=0; j < 6; j++) image[i++] = (byte)0x00;
				image[i++] = (byte)0xfe;
				image[i++] = (byte)nTrack;
				image[i++] = (byte)nSide;
				image[i++] = (byte)nSector;
				image[i++] = (byte)0x01;
				int crc1 = Utilities.crc16_get(image, nOffset+6, 5);
				image[i++] = (byte)((crc1>>8)&0xff);
				image[i++] = (byte)(crc1&0xff);
				for (int j=0; j < 11; j++) image[i++] = (byte)0xff;
				for (int j=0; j < 6; j++) image[i++] = (byte)0x00;
				image[i++] = (byte)0xfb;
				for (int j=0; j < 256; j++) image[i++] = sector[j];
				int crc2 = Utilities.crc16_get(image, nOffset+30, 257);
				image[i++] = (byte)((crc2>>8)&0xff);
				image[i++] = (byte)(crc2&0xff);
				for (int j=0; j < 45; j++) image[i++] = (byte)0xff;
			}
			else {
				index = (nSector*5) % sectpertrack;
				nOffset = 40 + index*340;

				int i=nOffset;				
				for (int j=0; j < 10; j++) image[i++] = (byte)0x00;
				for (int j=0; j < 3; j++) image[i++] = (byte)0xa1;		
				image[i++] = (byte)0xfe;
				image[i++] = (byte)nTrack;
				image[i++] = (byte)nSide;
				image[i++] = (byte)nSector;
				image[i++] = (byte)0x01;
				int crc1 = Utilities.crc16_get(image, nOffset+10, 8);
				image[i++] = (byte)((crc1>>8)&0xff);
				image[i++] = (byte)(crc1&0xff);
				for (int j=0; j < 22; j++) image[i++] = (byte)0x4e;
				for (int j=0; j < 12; j++) image[i++] = (byte)0x00;
				for (int j=0; j < 3; j++) image[i++] = (byte)0xa1;		
				image[i++] = (byte)0xfb;
				for (int j=0; j < 256; j++) image[i++] = sector[j];
				int crc2 = Utilities.crc16_get(image, nOffset+54, 260);
				image[i++] = (byte)((crc2>>8)&0xff);
				image[i++] = (byte)(crc2&0xff);
				for (int j=0; j < 24; j++) image[i++] = (byte)0x4e;
			}
		}
	}
