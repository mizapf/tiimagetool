...
public class WriteCFCardAction extends Activity {
	...
	public void go() {
		...
		if (rwd.confirmed()) {
			String[] commands = isMac ? rwd.getWrappedCommandLine(true) : rwd.getCommandLine();
			if (commands == null || commands.length < 3) {
				JOptionPane.showMessageDialog(m_parent, "Abord command. Command line was not set up correctly.", "CF card writing", JOptionPane.ERROR_MESSAGE);
			} else {
				try {
